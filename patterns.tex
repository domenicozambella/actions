...
We say that $\phi'(\bar x\,;z)$ is an \emph{alphabetic variant\/} of the formula $\phi(\bar x\,;z)$ if it is obtained by replacing the variables effectively occurring in $\phi(\bar x\,;z)$ by other variables among those in $z$.
...
This is often confused with either $\U^x/{\equiv_{\Delta,M}}$ or $\N/{\equiv_{\Delta,M}}$, with a preference for the latter.
...
\begin{proof}
  ...
  Up to replacing $\phi(\bar x\,;z)$  and $\alpha(z)$ with an alphabetic variant, we can assume that the $z$-variables that effectively occur in $\phi(\bar x\,;z)$ and $\alpha(z)$ are disjoint from those that occurin $\psi(\bar x\,;z)$ and $\beta(z)$ .
...
Let $\hat m$ be an enumeration of $M$ and $|z|=|\hat m|$.
We define
...
When also the converse of the implication in \# holds, we say that $f$ is a \emph{partial p-isomorphism.}
For the back-and-forth argumwent in the following poroof we need to take inverse of partial p-isomorphisms.
...
For a more convenient notation, $x$ is taken of infinite length~--~then, up to equivalence, all formulas in $L_z(\U)$ have the form $\phi(a\,;z)$ for some $\phi(x\,;z)\in L$ and $a\in\U^x$.
...
\begin{proof}
  ...
  Let $p(z)$ be the type containing $\phi(\hat m\cdot z\cdot a)$ for all $\phi(x)\in L$ and $a\in N^x$ such that $\phi(\hat m\cdot ga)$.
  We prove that $p(z)$ is strongly finitely satisfied in $M$.
...
In this section we show that ${\rm End}(\N)$, endowed with finer topology, is homeomorphic to the Ellis semigroup associated to the action %of ${\rm Aut}(M)$ on $S_{x,\Delta}(M)$.
% We prefer to work with the action of 
${\rm Aut}(N/\{M\})$ on $N^x$.
Though it is more common to work with the action of ${\rm Aut}(M)$ on $S_{x,\Delta}(M)$, we take the liberty to choose the former~--~the two actions being essentially equivalent.
...
This we call the {logic topology.}
