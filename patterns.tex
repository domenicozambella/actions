...
Finally, let $\Delta$ be the set of positive Boolean combinations of formulas in $\ \bigcup_{x'\in V}\Delta(x'\,;z)$.
The choice of $\Delta$ is justified in Remark~\ref{rem_first}.

Let $\bar a,\bar a'\in\big(\U^x\big)^n$ and $B\subseteq\U$.
...
Let $b,b'\in\U^z$ and $A\subseteq\U$.
We write \emph{$b\equiv_{A,\Delta}b'$} if $\phi(\bar a\,;b)\leftrightarrow\phi(\bar a\,;b')$ holds for every $\phi(\bar x,z)\in\Delta$ and $\bar a\in A^{\bar x}$.
...
For every formula $\phi(x_0,\dots,x_{n-1}\,;z)\in\Delta$ and every formula $\alpha(z)\in L$ the language $L^{\rm\! p}$ contains the $n$-ary relation \emph{$r_{\phi\,;\alpha}$.}
...
A total p-morphism is called a p-endomorphism.
We write ${\rm End}(\N)$ for the set of p-endomorphisms of $\N$.
We endow ${\rm End}(\N)$ with the topology that has as basic closed sets those of the form 

\ceq{\hfill\Big\{\ f\in{\rm End}(\N)}{:}{\N\ \models\ r_{\phi\,;\alpha}\big(f\!a_1,\dots,f\!a_n,c_1,\dots,c_m\big)\Big\}}

for $\phi(x\,;z)\in\Delta$, $\alpha(z)\in L$, and $a_1,\dots,a_n,c_1,\dots,c_m\in\N$.
...
\begin{fact}
  With the above topology ${\rm End}(\N)$ is T$_1$ (singletons are closed) and compact.
...
\begin{proof}
  ...
  % \ceq{\hfill\bigcap_{i<\lambda}\Big\{\ f\in{\rm End}(\N)}{:}{\N\ \models\ r_{\phi\,;\alpha}\big(f\!a_1,\dots,f\!a_{n_i},a'_1,\dots,a'_m_{i}\big)\Big\}}
...
\begin{proof}
  ...
  In 3 we prove the continuity of the first map and in 4 the continuity of the second.
  ...
  Clearly $p(z)$ contains $p_{\hat m}(z)$ and is complete over $N,\Delta$.
  ...
  Therefore $\hat m$ satisfies $\alpha(z)\rightarrow\phi_k(f_ba_k\,;z)$ and all its alphabetic variants.
  Therefore $\alpha(M^z)\subseteq\phi_k(f_ba_k;M^z)$ and $r_{\psi\,;\alpha}(f_ba_1,\dots,f_ba_n)$ follows.
  ...
  \ceq{\#}{=}{\Big\{f\ :\ r_{\psi\,;\top}\big(f\bar a,c\big)\Big\}}

  where $\psi(\bar x,y;z)$ and $c$ are choosen so that $r_{\psi,\top}(\bar x,c)$ is equivalent to $\phi(\bar x\,;\hat m)$.
  ...
  The formula $\psi(\bar x,y;z)$ is obtained from $\phi(\bar x\,;z)$ by replacing the variables in the tuple $z$ with variables in the tuple $y$.
...
In this section $|x|=1$.
We could assume that also $|z|=1$ but, we prefer to stick to the very same tuple of variables $z=\langle z_i:i<\lambda\rangle$ used above.
We may write $z\cdot x$ for the tuple $z\cdot x$.
% We write $z\cdot x$ for the tuple $\langle z_0\cdot x,\, z_1,\, z_2,\, \dots\rangle$.
% We write $z\cdot x$ for the term $z_0\cdot x$ with the same of sort $x$.
In this section $\Delta(x,z)$ contains $L_z$ and the formulas $\phi(z\cdot x)$ for all $\phi(x)\in L$.
% Note that $b/{\equiv_{N,\Delta}}$ only depends on $b_0$.

\begin{fact}
  Let $b\in\B$ be given.
  Let $f:\N\to\N$ be the function that maps $a\mapsto b\cdot a$.
  Then $f$ is a p-morphism.
\end{fact}

\begin{proof}
  We prove that $f$ is well-defined.
  Pick any two $a\equiv_{\Delta,M} a'$ in $\N$.
  As $b\nonfork_MN$ then $a\equiv_{\Delta,M,b}a'$, it follows that $b\cdot a\equiv_{\Delta,M}b\cdot a'$ as required.
  
  Now, assume $r_{\psi,\alpha}(a_1,\dots,a_n)$.
  We prove $r_{\psi,\alpha}(f\!a_1,\dots,f\!a_n)$.
  Assume that for some $i_{h,k}\in\{1,\dots,n\}$

  \ceq{\hfill\psi(x_1,\dots,x_n\,;z)}{=}{\bigvee_{h=1}^m\bigwedge_{k=1}^m\phi_{h,k}(\,z\cdot\, x_{i_{h,k}})\wedge\phi'_{h,k}(z)}

  for some $\phi_{h,k}(z_0\,;z),\phi'_{h,k}(z)\in\Delta$.

  From $r_{\psi\,;\alpha}(a_1,\dots,a_n)$, after re-indexing, we obtain that $\alpha(M^z)\subseteq\phi_k(M^z\cdot a_k)\cap\phi'_k(M^z)$ for $k=1,\dots,m$.
...
\begin{proof}
  ...
  Then $a\equiv_{\Delta,M}b$ follows.
...
A \emph{p-retraction} of $\U^x$ onto $\A$ is a total p-morphism $R$ that coincides with $\equiv_{\Delta,M}$ on $\A$, i.e.\@ such that $R\cap (\A\times\A)\ \ =\ \ ({\equiv_{\Delta,M}})\cap (\A\times\A)$.
