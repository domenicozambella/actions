...
Dually, $\D$ is $G$-thick \emph{relative\/} to $q(x)$ if $G{\cdot}\D$ is finitely consistent with $q(x)$.
...
\begin{proposition}\label{prop_StabP}\ 
  ...
  \ceq{\hfill G^{1\!1}}{=}{\big\{g\in G\ :\ \D\cap g{\cdot}\D\neq\varnothing\textrm{ for every }\D\neq\varnothing\text{ intersection of sets in }\Sigma_G(x) \big\}.}
\end{proposition}
...
\begin{proof}
  ...
  Then $\varphi(x)\wedge g^{-1}{\cdot}\varphi(x)$ is inconsistent for some $\varphi(x)$ that is a conjunction of formulas in $p$.
...
\begin{proposition}\label{prop_Gsyndetic_Hthick1}
  ...
  In particular all types in $Q_G$ are $G^1$-wide and $G^{1\!1}$-wide.
\end{proposition}
...
\begin{definition}\label{def_G00}
  Let $\Phi^0$ be the collection of sets that are equivalence classes of a bounded $G$-invariant $\BDelta(\Z)$-definable equivalence relation.
  ...
  \ceq{\hfill\emph{$G^{00}$}}{=}{{\rm Stab}(\Phi^{00})}\medrel{=}$\displaystyle\bigcap\big\{{\rm Stab}(\Y)\ :\  \Y\in\Phi^{00}\}$.\medskip
...
\begin{proposition}
  ...
  \ceq{\hfill G^1,\ G^{1\!1}}{\leq}{G^{00}}\medrel{\leq}$G^0$.
...
\begin{proof}
  ...
  We prove  $G^1\leq G^{00}$; the proof of $G^{1\!1}\leq G^{00}$ is similar.
  ...
  Then this type extends to some $p(x)\in Q_G$.
  By $G^1$-in\-variance $p(x)$ entails also $k{\cdot}\Y$ for every $k\in G^1$.
...
\begin{corollary}[ (with Alex Kruckman)]\label{corol_str_gen}
  The type ${\mathstrut}^{\rm s}\kern-.1ex\Sigma_G(x)$ is finitely consistent, strongly $G$-syn\-detic, and $G$-invariant.
  Moreover, \smallskip

  \ceq{\hfill{\mathstrut}^{\rm s}\kern-.1ex\Sigma_G(x)}{=}{\bigcap_{q\in Q_G}q(x)},\smallskip
...
\begin{proposition}[ (with Alex Kruckman)]\label{prop_coalese}\ 
  The equivalent conditions in Theorem~\ref{thm_coalesce} are also equivalent to\smallskip

  \ceq{5.\hfill\Sigma_G(x)}{=}{\bigcap\Big\{p(x)\in S_\Delta(\Z)\ :\ p(x)\text{ is }G\text{-invariant}\Big\}}
\end{proposition}
...
\ceq{\hfill\varphi(x,a)}{=}{r(x,a_1)\leftrightarrow r(x,a_2)\leftrightarrow\cdots\cdots\leftrightarrow r(x,a_n)}
...
\begin{theorem}[ (with Alex Kruckman)]
